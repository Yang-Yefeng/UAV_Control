...
if __name__ == '__main__':
    ...
    while uav.time < uav.time_max:
        ...
        if IS_IDEAL:
            dde = np.dot(uav.dot_f1(), uav.rho2()) + \
                  np.dot(uav.f1(), uav.f2() + np.dot(uav.h(), ctrl_in.control))  # - dot2_rhod
        else:
            dde = np.dot(uav.dot_f1(), uav.rho2()) + \
                  np.dot(uav.f1(), uav.f2() + np.dot(uav.h(), ctrl_in.control)) + \
                  delta_obs
        '''2. 计算 tk 时刻误差信号'''
        ...
        data_block = {'time': uav.time,
                      ...
                      'd_in': np.dot(uav.f1(), np.array([uncertainty[3], uncertainty[4], uncertainty[5]])) - dot2_rhod,
                      'd_in_obs': delta_obs,
