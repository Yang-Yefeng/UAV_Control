...
if __name__ == '__main__':
    uav = UAV(param)
    ctrl_in = backstepping(ctrl0=np.array([0, 0, 0]).astype(float), dt=uav.dt, k_bc=np.array([1, 1, 1]))
    ...
    while uav.time < uav.time_max:
        ...
        uncertainty = generate_uncertainty(time=uav.time, is_ideal=True)
        ...
        if OBSERVER == 'neso':
            syst_dynamic = np.dot(uav.dW(), uav.rho2()) + np.dot(uav.W(), uav.f2()) + np.dot(uav.W(),
                                                                                             np.dot(uav.J_inv(),
                                                                                                    ctrl_in.control))
        ...
        ctrl_in.control_update(e1=e_rho, de1=de_rho, f=uav.A_rho(), g=uav.B_rho(), dd_ref=dot2_rhod, obs=delta_obs)
